...
DEFAULT_STYLE = (
    "Read this aloud in a clear, natural voice, professional and measured. Do not add any words: "
)
...
def synth_all(texts: list[str], key: str, outdir: Path, **kw) -> dict[str, tuple[Path, float]]:
    ...
    cache = Path(os.environ.get("REPOCAST_CACHE_DIR", Path.home() / ".cache" / "repocast")) / "tts"
    cache.mkdir(parents=True, exist_ok=True)
    audio: dict[str, tuple[Path, float]] = {}
    for i, t in enumerate(dict.fromkeys(texts)):  # de-dup, keep order
        p = outdir / f"cue{i:03d}.wav"
        identity = json.dumps({"text": t, **kw}, sort_keys=True).encode()
        cached = cache / f"{hashlib.sha256(identity).hexdigest()}.wav"
        if cached.is_file():
